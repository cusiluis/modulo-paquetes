...
<head>
  ...
  <title>Editar Opción de Paquete</title>
    ...
    <main class="app-main">
      <section class="content header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1>Editar Opción de Paquete</h1>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="content">
        <div class="container-fluid">
            ...
            <form method="POST" action="<?= BASE_URL ?>paquetes/edita_opciones">
              <input type="hidden" name="id" value="<?= $opcion['id'] ?>">
              <div class="mb-3">
                <label>Título</label>
                <input type="text" name="titulo" class="form-control" value="<?= htmlspecialchars($opcion['titulo']) ?>" required>
              </div>
              <div class="mb-3">
                <label>Nombre del Portal</label>
                <input type="text" name="nombre_portal" class="form-control" value="<?= htmlspecialchars($opcion['nombre_portal']) ?>">
              </div>
              <div class="mb-3">
                <label>Descripción del Portal</label>
                <textarea name="descripcion_portal" class="form-control"><?= htmlspecialchars($opcion['descripcion_portal']) ?></textarea>
              </div>
              <div class="mb-3">
                <label>Días de duración</label>
                <input type="number" name="duracion_dias" class="form-control" value="<?= $opcion['duracion_dias'] ?>" required>
              </div>
              <div class="mb-3">
                <label>Subidas por día</label>
                <input type="number" name="veces_dia" class="form-control" value="<?= $opcion['veces_dia'] ?>" required>
              </div>
              <div class="mb-3">
                <label>Precio por subida (€)</label>
                <input type="number" step="0.001" name="precio_subida" class="form-control" value="<?= $opcion['precio_subida'] ?>" required>
              </div>
              <div class="mb-3">
                <label>Precio total (€)</label>
                <input type="number" step="0.01" name="precio_total" class="form-control" placeholder="(subidas/dia * precio por subida) * dias duracion" readonly value="<?= $opcion['precio_total'] ?>" required>
              </div>
